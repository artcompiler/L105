...
var transform = function () {
  ...
  function axes(node, options, resume) {
    ...
    visit(node.elts[0], options, function (e1, v1) {
      visit(node.elts[1], options, function (e2, v2) {
        var err = [].concat(e1).concat(e2);
        var val = Object.assign({ axes: v1 }, v2);
        resume(err, val);
      });
    });
  }
  ...
  function radarChart(node, options, resume) {
    ...
    function getAxes(val) {
      ...
      if (val.axes) {
        ...
        var hash = {};
        axes = val.reduce(function (axes, v) {
          var axis = v.axis && v.axis.trim();
          if (!hash[axis]) {
            hash[axis] = true;
  ...
  function data(node, options, resume) {
    ...
    if (node.elts.length === 0) {
      ...
      visit(node.elts[0], options, function (err1, val1) {
        var val = options.data && Object.keys(options.data).length !== 0 ? options.data : val1;
        resume([].concat(err1), val);
      });
    }
  }
